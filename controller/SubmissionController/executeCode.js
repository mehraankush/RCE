...
const processTestCase = async (
  ...
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(
      () => reject(new Error("Timeout: API call exceeded 10 seconds.")),
      10000
    );
  });
  ...
  while (true) {
    try {
      const userResultPromise = await axios.get(
      ...
      console.log(
        "userResultPromise.data.submissions",
        userResultPromise.data.submissions
      );

      const CheckTLE = userSubmissionsStatus.some(
      ...
      const allAccepted = userSubmissionsStatus.every(
        (submission) => submission.status.id === 3 || submission.status.id === 4
      );
